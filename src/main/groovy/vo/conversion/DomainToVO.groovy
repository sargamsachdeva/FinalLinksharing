...
class DomainToVO {
    ...
    static PostVO post(Resource resource) {
        ...
        return new PostVO(
                ...
                userPhoto: post[11],
                postDate: post[12],
                resourceRating: resource.calculateAverageRating()
        )
    }

    static PostVO postNew() {
    ...
    static List<PostVO> getUnreadreadingItems(User user) {
        List<PostVO> readingItemsVO = []
        user.unreadItems().each {
            readingItemsVO.add(new PostVO(
                    resourceID: it.resource.id,
                    description: it.resource.description,
                    topicID: it.resource.topic.id,
    ...
    static List<TopicVO> userSubscriptions(User user,SearchCO searchCO) {
        if (user) {
            List<TopicVO> userSubscriptionsVO = []
            user.userSubscriptions(searchCO).each {
