...
class User {
    ...
    static constraints = {
        ...
        confirmPassword(nullable: true, blank: true, bindable: true)
    }
    ...
    Integer getTotalSubscriptions() {
        return Subscription.countByUser(this) ?: 0
    }
    ...
    def userSubscriptions(SearchCO searchCO) {
        return Subscription.createCriteria().list(max:searchCO.max,offset:searchCO.offset) {
            projections {
                'topic' {
                    property('id')
                    property('topicName')
...
/*    static List<User> getUserWithUnreadResources() {
    ...
    def unreadItems(){
        ...
        return ReadingItem?.findAllByUserAndIsRead(this,false,[max:5])
    }
}
