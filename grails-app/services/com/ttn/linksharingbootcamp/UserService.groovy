...
class UserService {
    ...
    User registerUser(UserCO userCO, def sourceFile) {
        ...
        log.info("password-->${userCO.password}")

        log.info("password-->${userCO.confirmPassword}")
        if (sourceFile.empty) {

            if (user.save(flush: true)) {
                return user
            } else {
                log.info("1...."+user.errors.allErrors)
                return null
            }
            ...
            if (user.save(flush: true)) {
                ...
                log.info("2......"+user.errors.allErrors)
                return null
            }
    ...
    def sendUnreadItemsMail() {
        User.getUserWithUnreadResources().each { user ->
            emailService.sendUnReadResourcesMail(user, User.getUnreadResources(user))
        }
    }
