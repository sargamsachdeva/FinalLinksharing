    ...
    def showSeriousness = { attrs, body ->
        ...
        if (user) {
            if (topic) {
                ...
                if (subscription) {
                    out << "${g.select(name:'seriousness', id: 'seriousness', from: enums.Seriousness.values(), class: 'btn btn-xs btn-default dropdown-toggle seriousness','data-topicId': topicId, value:subscription.seriousness)}" }
            }
        }
    }
